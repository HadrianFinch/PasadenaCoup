...
const actionMessages = {
    ...
    'exchange': (idx) => `{${idx}} attempted to inquisite the deck`,
    'interrogate': (idx, target) => `{${idx}} attempted to inquisite {${target}}`,
    'contessa': (idx, target) => `{${idx}} attempted to contessa {${target}}`,
...
module.exports = function createGame(options) {
    ...
    function challenge(playerIdx, challengedPlayerIdx, challengedRole) {
        ...
        if (proof != null) {
            ...
            var message = format('{%d} incorrectly challenged {%d}', playerIdx, challengedPlayerIdx,);
            ...
            if (playerState.influenceCount <= 1) {
                ...
                addHistory('incorrect-challenge', curTurnHistGroup(), '%s, {%d} got a new card.', message, playerIdx);
    ...
    function playAction(playerIdx, actionState) {
        ...
        else if (actionState.action == 'contessa') 
        {
            target = state.players[actionState.target];
